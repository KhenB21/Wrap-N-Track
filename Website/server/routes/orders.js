...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Missing required fields for customer order" });
    }
    ...
    try {
      ...
      const customerAddress =
        customerResult.rows[0].address || "Unknown Address";
      ...
      if (!orderResult.rows.length) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to create customer order" });
      }
        ...
        for (const product of products) {
          ...
          if (product.itemName && !product.sku) {
            const skuResult = await client.query(
              "SELECT sku FROM inventory_items WHERE name = $1",
              [product.itemName]
            );
            if (skuResult.rows.length === 0) {
              await client.query("ROLLBACK");
              return res.status(400).json({
                error: `Product not found in inventory: ${product.itemName}`,
              });
            }
          ...
          if (!product.sku || product.quantity === undefined) {
            // Check quantity for undefined specifically
            await client.query("ROLLBACK");
            return res.status(400).json({
              error: "Each product must have a valid SKU and quantity",
            });
          }
...
router.put("/:order_id", async (req, res) => {
  ...
  if (!products || !Array.isArray(products) || products.length === 0) {
    return res
      .status(400)
      .json({ error: "Products are required and must be a non-empty array." });
  }
  for (const product of products) {
    if (
      !product.sku ||
      ...
      return res.status(400).json({
        error: "Each product must have a valid SKU and a positive quantity.",
      });
    }
  }
  ...
  try {
    ...
    if (products && Array.isArray(products) && products.length > 0) {
      for (const product of products) {
        ...
        if (inventoryItemResult.rows.length === 0) {
          await client.query("ROLLBACK");
          return res.status(400).json({
            error: `Product with SKU ${product.sku} not found in inventory for cost calculation.`,
          });
        }
        const unitPrice =
          parseFloat(inventoryItemResult.rows[0].unit_price) || 0;
    ...
    for (const product of products) {
      ...
      if (inventoryResult.rows.length === 0) {
        await client.query("ROLLBACK");
        return res.status(400).json({
          error: `Product with SKU ${product.sku} not found in inventory.`,
        });
      }
      ...
      if (availableQuantity < product.quantity) {
        await client.query("ROLLBACK");
        return res.status(400).json({
          error: `Insufficient stock for SKU ${product.sku}. Available: ${availableQuantity}, Requested: ${product.quantity}.`,
        });
      }
    }
    ...
    if (updateFields.length === 0 && status === undefined) {
      // Ensure at least status is being updated or some field
      await client.query("ROLLBACK");
      return res.status(400).json({
        error: "No fields to update for the order, or status is missing.",
      });
    }
    ...
    const updateOrderQuery = `UPDATE orders SET ${updateFields.join(
      ", "
    )} WHERE order_id = $${valueIndex} RETURNING *`;
  ...
  try {
    ...
    if (normalizedDBStatus !== "pending" && normalizedDBStatus !== "tobepack") {
      ...
      return res.status(403).json({
        success: false,
        message:
          'Order cannot be cancelled. Only orders with status "Pending" or "To Be Pack" can be cancelled.',
      });
    }
    ...
    console.log(
      `[CancelOrder-${order_id}] Fetched products for restocking:`,
      JSON.stringify(orderProducts)
    );

    if (orderProducts.length === 0) {
      console.log(
        `[CancelOrder-${order_id}] No products found in order_products to restock.`
      );
    }
    ...
    for (const product of orderProducts) {
      console.log(
        `[CancelOrder-${order_id}] Processing product SKU: ${product.sku}, Quantity to restock: ${product.quantity}`
      );
      if (typeof product.quantity !== "number" || product.quantity <= 0) {
        console.error(
          `[CancelOrder-${order_id}] Invalid quantity for SKU ${product.sku}: ${product.quantity}. Skipping restock for this item.`
        );
        continue;
      }
      ...
      console.log(
        `[CancelOrder-${order_id}] Inventory update result for SKU ${product.sku}: rowCount = ${updateInventoryResult.rowCount}`
      );
      if (updateInventoryResult.rowCount === 0) {
        console.warn(
          `[CancelOrder-${order_id}] WARN: No inventory item found for SKU ${product.sku} or quantity_in_stock was not updated.`
        );
      }
    }
    ...
    console.log(
      `[CancelOrder-${order_id}] Updating order status to 'Cancelled'.`
    );
    await client.query(
    ...
    await client.query("COMMIT");
    console.log(
      `[CancelOrder-${order_id}] Transaction committed. Responding to client.`
    );
    res.json({
      success: true,
    ...
    console.error(
      `[CancelOrder-${order_id}] Error during cancellation process:`,
      error.message,
      error.stack
    );
    console.log(
      `[CancelOrder-${order_id}] Rolling back transaction due to error.`
    );
    try {
      await client.query("ROLLBACK");
      ...
      console.error(
        `[CancelOrder-${order_id}] Error during ROLLBACK:`,
    ...
    res.status(500).json({
      success: false,
      message: "Failed to cancel order due to an internal error.",
      details: error.message,
    });
  } finally {
    client.release();
...
router.get("/gift-details", async (req, res) => {
  try {
    ...
    const result = await client.query(
      "SELECT * FROM order_gift_details ORDER BY name"
    );
    client.release();
  ...
  try {
    ...
    if (ordersToUpdate.length === 0) {
      ...
      return res.status(200).json({
        message: "No orders found needing total_cost update.",
        updatedCount: 0,
      });
    }
    ...
    res.status(500).json({
      error: "Failed to backfill total_cost for orders.",
      details: error.message,
    });
  } finally {
    client.release();
...
router.get("/archived", (req, res) => {
  ...
  db.query(query, (err, results) => {
    if (err) {
      ...
      return res
        .status(500)
        .json({ message: "Failed to fetch archived orders" });
    }
    res.json(results);
  });
});
