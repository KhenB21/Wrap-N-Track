// Stock In / Stock Out rules shared by every mobile screen that moves stock.
// Keep in sync with Website/client/src/constants/stockReasons.js and the
...
// Returns an error message, or null when the reason is complete. Pass `action`
// so a reason picked for Stock In is not accepted after switching to Stock Out.
export const validateStockReason = (reason, notes, action) => {
  if (!reason || (action && !(STOCK_REASONS[action] || []).includes(reason))) {
    return 'Select a reason for this stock adjustment.';
  }
