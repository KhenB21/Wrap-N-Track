...
export default function EditProductScreen({ navigation }) {
  ...
  const { product, mode = 'edit' } = route.params;
  ...
  const [formData, setFormData] = useState({
    ...
    uom: product?.uom || '',
    conversion_qty: product?.conversion_qty?.toString() || '',
  ...
  const isAddStockMode = mode === 'addStock';

  useEffect(() => {
    if (isAddStockMode) {
      navigation.setOptions({ title: 'Add Stock' });
    }
  }, [isAddStockMode]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      setLoading(true);
      // Implement API call to update product or add stock
      const action = isAddStockMode ? 'Stock added' : 'Product updated';
      Alert.alert('Success', `${action} successfully`, [
        { text: 'OK', onPress: () => navigation.goBack() }
      ]);
    } catch (error) {
      Alert.alert('Error', `Failed to ${isAddStockMode ? 'add stock' : 'update product'}`);
    } finally {
      setLoading(false);
    }
  };
    ...
    <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
          ...
          <Card.Content>
            ...
            >
              ...
              <Text style={[styles.imageText, { color: theme.colors.onSurfaceVariant }]}>
                Tap to {product?.image_data ? 'change' : 'add'} image
              </Text>
            </TouchableOpacity>
          </Card.Content>
        </Card>
        ...
        <Card style={[styles.card, { backgroundColor: theme.colors.surface }]}>
          <Card.Content>
            <Text style={[styles.sectionTitle, { color: theme.colors.onSurface }]}>
              {isAddStockMode ? 'Add Stock' : 'Stock Information'}
            </Text>
            ...
            <TextInput
              label={isAddStockMode ? 'Quantity to Add *' : 'Initial Quantity *'}
      ...
      <View style={styles.actionButtons}>
        ...
        >
          {isAddStockMode ? 'Add Stock' : 'Update Product'}
        </Button>
      </View>
    </View>
  );
